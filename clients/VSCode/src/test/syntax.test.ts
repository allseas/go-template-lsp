...
//these tests are written with the support of AI
suite("Syntax Highlighting Test Suite", () => {
    ...
    after(() => {
        vscode.window.showInformationMessage(
            "All syntax highlighting tests done!",
        );
    });
    ...
    test("Action delimiters are highlighted", () => {
        const line = "{{ .Foo }}";
        assertScope(
            getScopes(grammar, line, 0),
            "punctuation.definition.embedded.begin.gotmpl",
        );
        assertScope(
            getScopes(grammar, line, 8),
    ...
    test("Variable reference is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ $myVar }}", 3),
            "variable.other.gotmpl",
        );
    });
    ...
    test("Standalone dot is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ . }}", 3),
            "variable.language.dot.gotmpl",
        );
    });

    test("Field access is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ .Name }}", 3),
            "variable.other.member.gotmpl",
        );
    });
    ...
    test("Pipe operator is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ .Name | html }}", 9),
            "keyword.operator.pipe.gotmpl",
        );
    });

    test("Boolean true is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ true }}", 3),
            "constant.language.boolean.gotmpl",
        );
    });

    test("Boolean false is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ false }}", 3),
            "constant.language.boolean.gotmpl",
        );
    });
    ...
    test("Nil is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ nil }}", 3),
            "constant.language.nil.gotmpl",
        );
    });

    test("Integer number is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ 42 }}", 3),
            "constant.numeric.gotmpl",
        );
    });

    test("Float number is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ 3.14 }}", 3),
            "constant.numeric.gotmpl",
        );
    });
    ...
    test("Double-quoted string is highlighted", () => {
        assertScope(
            getScopes(grammar, '{{ "hello" }}', 3),
            "string.quoted.double.gotmpl",
        );
    });

    test("Raw string is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ `raw` }}", 3),
            "string.quoted.other.raw.gotmpl",
        );
    });

    test("Char literal is highlighted", () => {
        assertScope(
            getScopes(grammar, "{{ 'a' }}", 3),
            "string.quoted.single.gotmpl",
        );
    });
    ...
    test("Escape sequence inside string is highlighted", () => {
        assertScope(
            getScopes(grammar, '{{ "\\n" }}', 4),
            "constant.character.escape.gotmpl",
        );
    });
    ...
    test("Dot inside range block is highlighted across lines", () => {
        ...
        const scopes = (() => {
            for (const token of line2.tokens) {
                if (token.startIndex <= 3 && 3 < token.endIndex)
                    return token.scopes;
            }
            return [];
        })();
