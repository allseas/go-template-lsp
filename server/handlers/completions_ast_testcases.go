...
var completionTestCases = []completionTestCase{
	...
	// pipe filtering - model fields
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"html", "js", "urlquery", "len", "not", "and", "eq"},
	},
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"not", "and", "or", "html", "len"},
	},
	{
	...
	// pipe filtering - model methods
	{
		...
		isInvoked:   true,
		withType:    true,
		contains:    []string{"html", "js", "len", "not", "and", "eq"},
		notContains: []string{".Format"},
	},
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"eq", "lt", "gt", "not", "html", "len"},
	},
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"not", "and", "len", "html"},
	},
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"html", "len", "not", "and"},
	},
	...
	// multi-stage pipe chaining
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"len", "js", "not", "eq"},
	},
	{
		name:        "not after bool field - all builtins shown",
		src:         `{{.Paid | not | }}`,
		subStr:      "}}",
		offsetAdj:   -1,
		isInvoked:   true,
		withType:    true,
		contains:    []string{"and", "or", "len", "html"},
		notContains: []string{".Oper"},
	},
	// dot piped directly
	{
	...
	// builtin chained with model
	{
		...
		isInvoked: true,
		withType:  true,
		contains:  []string{"eq", "lt", "print", "html", "not"},
		notContains: []string{
			".Oper",
			".DisplayName",
			".ItemCount",
	...
	{
		name:      "invoked after string pipe - all builtins shown",
		src:       `{{.CustomerName | }}`,
		subStr:    "}}",
		offsetAdj: -1,
		isInvoked: true,
	...
	{
		name:       "inside range - string Item method piped, all builtins shown",
		src:        `{{range .Items}}{{.Label | }}{{end}}`,
		subStr:     "}}",
		occurrence: 1,
		offsetAdj:  -1,
	...
	{
		name:       "inside range - bool Item method piped, all builtins shown",
		...
		isInvoked:  true,
		withType:   true,
		contains:   []string{"not", "and", "html", "len"},
	},
	// scope switch - with
	{
	...
	{
		name:       "inside with - string Address method piped, all builtins shown",
		src:        `{{with .Address}}{{Line | }}{{end}}`,
		subStr:     "}}",
		occurrence: 1,
		offsetAdj:  -1,
	...
	{
		name:       "inside with - bool Address method piped, all builtins shown",
	...
	{
		name:      "after len pipe - all builtins shown",
		src:       `{{. | len | }}`,
		subStr:    "}}",
		offsetAdj: -1,
		isInvoked: true,
		withType:  true,
		contains:  []string{"eq", "lt", "print", "index", "js"},
	},
	{
		name:     "after not pipe - all builtins shown",
		src:      `{{not . | and . .}}`,
		subStr:   "a",
		withType: true,
		contains: []string{"and", "or", "not", "len", "html"},
	},
	{
		name:       "after html pipe - all builtins shown",
		src:        `{{html . | len .}}`,
		subStr:     "l",
		occurrence: 1,
		withType:   true,
		contains:   []string{"len", "index", "and", "not"},
	},
	{
